...
<body>
    <div class="main-container" style="max-width: 1200px; margin: auto;">
        <div class="easyui-panel" title="Gerenciamento de Pedidos" style="padding:10px;">
            ...
            <div style="margin-bottom:10px;">
                <a onclick="abrirDialogInclusaoPedido()" class="easyui-linkbutton" data-options="iconCls:'icon-add'">Incluir Pedido</a>
    ...
    <div id="dlg-pedido-buttons">
        <a class="easyui-linkbutton" iconCls="icon-save" onclick="salvarPedido()" style="width:90px">Salvar</a>
        <a class="easyui-linkbutton" iconCls="icon-cancel" onclick="$('#dlg-pedido').dialog('close')" style="width:90px">Cancelar</a>
    </div>
    ...
    <div id="dlg-item-pedido-buttons">
        <a class="easyui-linkbutton" iconCls="icon-save" onclick="salvarItemPedido()" style="width:90px">Salvar</a>
        <a class="easyui-linkbutton" iconCls="icon-cancel" onclick="$('#dlg-item-pedido').dialog('close')" style="width:90px">Cancelar</a>
    </div>
</body>
...
<script>
    ...
    function formatActionPedido(value, row) {
        var btnModificar = `<a class="easyui-linkbutton" data-options="iconCls:'icon-edit',plain:true" onclick="abrirDialogModificacaoPedido(${row.num_pedido})">Modificar</a>`
        var btnExcluir = `<a class="easyui-linkbutton" data-options="iconCls:'icon-remove',plain:true" onclick="excluirPedido(${row.num_pedido})">Excluir</a>`
        return btnModificar + ' ' + btnExcluir
    }

    function formatActionItemPedido(value, row) {
        var btnModificar = `<a class="easyui-linkbutton" data-options="iconCls:'icon-edit',plain:true" onclick="abrirDialogModificacaoItem(${row.num_pedido}, ${row.num_seq_item})"></a>`
        var btnExcluir = `<a class="easyui-linkbutton" data-options="iconCls:'icon-cancel',plain:true" onclick="excluirItemDoPedido(${row.num_pedido}, ${row.num_seq_item})"></a>`
        return btnModificar + ' ' + btnExcluir
    }
    ...
    function salvarPedido() {
        $('#fm-pedido').form('submit', {
            ...
            success: function(result) {
                ...
                } else $.messager.alert('Erro', res.message || 'Ocorreu um erro ao salvar.', 'error')
            }
        })
    }
        ...
        $('#fm-item-pedido').form('submit', {
            ...
            success: function(result) {
                ...
                if (res.success) {
                    ...
                    $('#dg_pedidos').datagrid('reload')
                } else $.messager.alert('Erro', res.message, 'error')
            }
        })
    }
